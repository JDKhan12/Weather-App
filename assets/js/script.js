...
// try {
      ...
      //     // Condition if response is 404 display invalid text and disable response
      //   } else if (response.status === 404) {
      //     throw `<h2>Invalid or ${searchInput.value} not found in database</h2>`;
      //   } else if (response.status >= 500) {
      //     throw `<h2>SERVER ERROR</h2>`;
      //   }
      // } catch (e) {
      //   weatherContainer.innerHTML = "";
      //   cityNotFound.style.display = 'block';
      //   cityNotFound.innerHTML = e;
      // }
  ...
  fetch(baseApi)
    .then(function (response) {
      ...
      // Condition if response is 404 display invalid text and disable response
        ...
        throw new Error (`<h2>Invalid or ${searchInput.value} not found in database</h2>`);
      } 
    })
    .then(function (data) {
      showWeather(data);
    ...
    .catch( function (err) { 
      weatherContainer.innerHTML = "";
      cityNotFound.style.display = 'block';
      cityNotFound.innerHTML = err.message;
    }) 
}

// const fetchData = async () => {
//   try {
//     fetch(`https://api.openweathermap.org/data/2.5/weather?q=${'delhddi'}&appid=${apiKey}`).then((res) => res.json()).then((res) => {
//       if (res.cod == 200) {
...
//       }
//     })
//     const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${'mumbai'}&appid=${apiKey}`);
